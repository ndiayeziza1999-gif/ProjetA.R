...
import { Link } from "react-router-dom"; // Importation indispensable
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          {displayedProducts.map((product) => (
            <div key={product.id} className="group flex flex-col">
              ...
              <Link to={`/product/${product.id}`} className="cursor-pointer">
                <h3 className="font-bold text-base mb-1 text-gray-900 hover:text-gray-600 transition-colors">
                  {product.name}
                </h3>
              </Link>
              ...
              <div className="flex flex-wrap gap-1">
                {product.sizes.map((size) => (
                  <span
                    key={size}
                    className="text-[10px] font-bold border border-gray-200 px-1.5 py-0.5 text-gray-400 hover:border-black hover:text-black transition-colors" >
                    {size}
                  </span>
                ))}
              </div>
